...
- [entryway] The standing pet-backpack Prop is 62 × 55 cm (--w 103.8, --h 91.3 on the 1184-unit stage), about 56% over a real 40 × 50 bag, because it takes the size S15's last frame shows. (105, 2026-09-26)
...
- [cinema] If the visitor leaves the Cinema while the Film Invitation dialog is open, closing it afterwards starts the reel errand in the hidden Cinema. (code review, 2026-09-26)
- [site] `styles.css`'s comment beside the `pointer-events: none !important` floor says the Cinema's `button.cinema-prop` rule out-ranks it on the gate; it cannot, and `cinema-room.ts`'s "the model is what refuses the click" now holds for the keyboard only. (code review, 2026-09-26)
- [site] The Entryway's costume layers (`data-cycle="costume"`, `assets/entryway/boy-costume-right.png`) are a third sheet kind that `docs/agents/scene-and-cycle-assets.md` does not name; `check-assets.test.mjs` was widened to admit them. (code review, 2026-09-26)
- [site] `styles.css` was rewritten from `\r\r\n` to `\r\n` endings in ticket 101, so `git blame` on it points at that commit for every line. (code review, 2026-09-26)
- [site] `src/world/arrival.ts` repeats the cats' heights (`CAT_HEIGHTS`) that `index.html`'s `data-height` already declares, and carries each Beat sheet's pixel metrics that belong in its manifest. (code review, 2026-09-26)
- [site] `src/world/cinema.ts` repeats `slice.errand ? nextPin(slice.errand, slice.pinned) : null` in four places, and `src/dom/entryway.ts` re-implements `actors.ts`'s layer setup for the costume sprites. (code review, 2026-09-26)
- [site] `src/dom/invitation.ts` keeps its open dialog in a module-level `let`, and reaches its link text and consent label through `querySelector` chains rather than ids. (code review, 2026-09-26)
- [site] Rewriting a junk hash with `location.replace` fires the browser's own `hashchange`, so `src/dom/rooms.ts` dispatches `hash-changed` twice for it. (code review, 2026-09-26)
- [cinema] The motion-off rule `.motion-off .is-rummaging .shelf-body` targets an element that does not exist (the shake is on `.shelf-art`); nothing shows because a rummage lasts one tick with motion off. (code review, 2026-09-26)
- [site] The code step (107) also bobs the cats in every Room, and the Film Invitation (108) carries its own six `film*` copy keys and an Apple TV link: both are wider than their tickets asked. (code review, 2026-09-26)
