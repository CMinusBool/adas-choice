...
The public GitHub Pages site contains no recipient email address or private email keys. It sends only a game ID (or, from the Cinema, a Film ID with the title and year the visitor saw), the fixed invitation action, explicit consent, a one-time bot-verification token, an idempotency UUID, and a honeypot field to the configured HTTPS Worker.
...
- Recipient and sender fixed in server secrets. Game and Film titles and action text come from server allowlists; a Film title is accepted only when it is that Film's own, in one of its two languages. No arbitrary subject, body, attachment, HTML or reply-to input is accepted.
